...
template<class ElementType>
void BST<ElementType>::copyBST(BSTNode<ElementType>* root, BSTNode<ElementType>* newroot)
{
	if (root == NULL)
		return;
	else
	{
		...
		if (root->left != NULL)
			newroot->left = new BSTNode<ElementType>(0);
		if (newroot->right != NULL)
			newroot->right = new BSTNode<ElementType>(0);

		copyBST(root->left, newroot->left);
		copyBST(root->right, newroot->right);
...
//
template<class ElementType>
void BST<ElementType>::deleteBST(BSTNode<ElementType>* root)
{
	if (root == NULL)
		return;
	deleteBST(root->left);
...
bool BST<ElementType>::insertR(const ElementType& anElement, BSTNode<ElementType>* current) {
	...
	// to do
	if (current == NULL)
		current = new BSTNode<ElementType>(anElement);
...
ElementType& BST<ElementType>::retrieveR(const ElementType& targetElement, BSTNode<ElementType>* current) const {
	...
	// to do
	if (current == NULL)
		throw ElementDoesNotExistException("TargetElement is not found in the binary search tree.");
